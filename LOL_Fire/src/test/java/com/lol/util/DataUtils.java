...
public class DataUtils extends TestExecutor {
	...
	{
		File = new FileInputStream(userDir + "\\src\\test\\java\\input\\Data.xlsx");
	...
	public void Logger_Println(int rowVal)
	{try
	{
		rowCount = sheet.getLastRowNum() + 1;
		int colCount = sheet.getRow(0).getLastCellNum();
		Row firstRow = sheet.getRow(0);
		Row currRow = sheet.getRow(rowVal);
		//logger.log(LogStatus.PASS, "Successfully Inserted Order details in the Grid from Row"+rowVal+": ");
		for (int j = 0; j < colCount; j++) 
		{
		//logger.log(LogStatus.INFO,  formatter.formatCellValue(firstRow.getCell(j)) + " = "+ formatter.formatCellValue(currRow.getCell(j)));
		map.put(formatter.formatCellValue(firstRow.getCell(j)), formatter.formatCellValue(currRow.getCell(j)));
		}
		//book.close();
	}
